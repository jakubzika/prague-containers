...
# ╔═╡ 0da9b14e-a60a-11ec-19f0-89f717d37de4
begin
...
# ╔═╡ 6bcb4635-3c2a-4300-af2b-317265075b83
begin
	include("src/data-conversion.jl")
end


# ╔═╡ 303540e9-38da-4218-9e2c-2ad20e470363
osmMap = get_map_data("map.osm")

# ╔═╡ 36b624bc-19d5-4900-9ea9-9330bb96307b
containers = DataConversion.loadContainers()

# ╔═╡ 31a3ba41-cc6f-44bb-99d5-69ccf94b2b2b
table = Shapefile.Table("TMBUDOVA_P_shp/TMBUDOVA_P.shp");

# ╔═╡ 35497134-e864-4aa0-ba72-ddebea96d0d7
table2 = Shapefile.Table("URK_SS_VyuzitiZakl_p_shp/URK_SS_VyuzitiZakl_p.shp");

# ╔═╡ 4b3fd213-fad7-43dc-892e-be1f65329e60
df = table |> DataFrame;

# ╔═╡ 62078cf4-3c1c-49ff-bc3d-39e76dc676d3
filter(df) do b
	b.TID_TMBUDO === 16289700
end

# ╔═╡ efbf415a-fda7-4b4c-9d61-45f54f0ea3bf
df2 = table2 |> DataFrame

# ╔═╡ 9f09387a-56a2-41db-a3ff-2564b5db4f8b
unique(df2.KOD)

# ╔═╡ 04ee7ec6-5a15-4e54-a839-c7242c672a9e
p = df[1,:geometry]

# ╔═╡ 561b0ca5-efbd-40d0-b1dd-84fc3c63987a
table3 = Shapefile.Table("554782.shp/BuildingPart.shp");

# ╔═╡ a1d906a2-f124-4cce-a250-f3cd35876156
df3 = table3 |> DataFrame

# ╔═╡ 6206d27a-4733-4cbd-970d-5cacd433bde8
populationCounts = let 
	temp = CSV.File("CR_L4_KU.csv", stringtype=String, decimal=',', types=[Int64, String, Int64, Float64]) |> DataFrame
	rename!(temp, :Id => :ID, :Name => :NAME)
	temp
end


# ╔═╡ 7afa6c3c-7b77-4f02-80fc-bbac1269edd4
exploratory = Shapefile.Table("/Users/jakubzika/Downloads/727300/KATASTRALNI_UZEMI_P.shp") |> DataFrame

# ╔═╡ 7c6cd884-6e24-493e-8a2d-cedfe95a5e1e
plot(exploratory.geometry)

# ╔═╡ 5436a4e8-627c-4ee4-a8f9-667aa966951c
data = let
	res = empty(exploratory)
	for id in populationCounts.ID
		t = Shapefile.Table("downloaded-data/land-registry/$(id)/KATASTRALNI_UZEMI_P.shp")
		append!(res, t)
	end
	res
end

# ╔═╡ 52785e91-6167-4d07-b9cb-1f007a5c01b8
maxCount = maximum(populationCounts.count)

# ╔═╡ 022bf53c-d468-406e-90a4-492441225d91
joinedPopulationCounts = innerjoin(data, populationCounts, on=:KATUZE_KOD => :ID)

# ╔═╡ 657e2a48-126b-4379-9b82-c7402e663f2a
begin
	colors = cgrad(:Blues_4)
	for r in eachrow(joinedPopulationCounts)
		normalized = r.count / maxCount
		plot!(r.geometry, color=colors[normalized])
	end
	# plot!(
	# 	joinedPopulationCounts.geometry, joinedPopulationCounts.count/maxCount, c= :Blues_4
	# )
	
	plot!(thickness_scaling=20)
end
...
# ╔═╡ Cell order:
# ╠═0da9b14e-a60a-11ec-19f0-89f717d37de4
# ╠═b1020e9e-50c3-4642-8245-885ed79f0e9e
# ╠═6bcb4635-3c2a-4300-af2b-317265075b83
# ╠═303540e9-38da-4218-9e2c-2ad20e470363
# ╠═36b624bc-19d5-4900-9ea9-9330bb96307b
# ╠═31a3ba41-cc6f-44bb-99d5-69ccf94b2b2b
