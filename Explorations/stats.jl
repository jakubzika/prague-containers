...
# ╔═╡ 1a2ce52a-a2c8-11ec-038a-e3cd3c243484
begin
	using Plots
	using Random
	using PlutoUI
...
# ╔═╡ 9672e89b-f521-40a2-9a5e-f1d0cc9b6e34
begin
	include("src/data-conversion.jl")
end

# ╔═╡ 808cd510-106b-4bee-ba28-4c2f326332bc
# begin
# 	using Pkg
# 	Pkg.activate()
# end

# ╔═╡ 6bfdaf09-76c1-4d9a-97d9-5c41fbc9c85e
plotly()

# ╔═╡ 495d8c9b-94e5-4045-a5b1-37b5c78772fd
md"""
## Load data
"""

# ╔═╡ 5d1a55d8-2b39-43d5-bdf8-8f724e6a817b
begin
	allContainers = DataConversion.loadContainers();
	isMeasuredPredicate(c) = !ismissing(c[:sensor_id]);
	measuredContainers = filter(isMeasuredPredicate,allContainers);
end;

# ╔═╡ a4542df8-eb5d-45aa-b951-818740bbde54
allContainers

# ╔═╡ 3038ffd8-0f66-47a2-914d-6515ea8226de


# ╔═╡ 69d97594-704b-4992-ad74-ad7bb9aa39f0
md"""
$(@bind containerIndex NumberField(1:nrow(measuredContainers)))
"""

# ╔═╡ 771f7f36-cd8c-40af-a6cf-347620a40665
measurements = DataConversion.getContainerMeasurements(measuredContainers[containerIndex, :].container_id);

# ╔═╡ 24fc9dce-9556-4339-b672-7f2d2fa24843
md"""
Select date range:

from: $(@bind fromDate DateField(default=today()))

to: $(@bind toDate DateField(default=today()))
"""

# ╔═╡ 273424aa-18a2-4e1d-b257-a65496466eaa
hourInterval = 1000 * 60 * 60

# ╔═╡ 0620dc7c-a8ca-4228-b2f5-8fdb75a6d53e
filterRows(m, from, to) = filter(r -> !isnothing(r.measured_at) && r.measured_at > from && r.measured_at < to, m)

# ╔═╡ c6ea6c8a-2a12-4d66-a877-56985fcf6c39
...
# ╔═╡ cfa39cf8-5dee-4b4a-8c3f-c5d51d6098ad
filRows = @pipe measurements |> filterRows(_, fromDate, toDate) |> filter(m -> m.percent_calculated > 90,_);

# ╔═╡ 1c4f7349-72d4-489a-bd37-0fa1c6537250
with_terminal() do
	fullnessFactor(measurements, fromDate, toDate)
end

# ╔═╡ cbb204c7-64f4-4a18-ab0e-10de09169220
function getFullnessFactorForContainer(containerId::String)
	println("Container id: $(containerId)")
	m = measurements = DataConversion.getContainerMeasurements(containerId)
	if(nrow(measurements) === 0)
		return 0
	end
	f = fullnessFactor(m, fromDate, toDate)
	return f
end

# ╔═╡ 7855eb51-43d6-4643-8715-a35e0826a64c
measuredContainers[1,:]

# ╔═╡ cc56cd93-3c5e-46a4-beef-91fbfdde64a1
factors = @pipe measuredContainers.container_id |> map(c -> (c,getFullnessFactorForContainer(c)),_)

# ╔═╡ 169140ca-8155-4a9b-b7de-60d9c92138b8

gContainers = GeoJSON.parsefile("/Users/jakubzika/School/Bachelor-thesis/Explorations/ZPK_O_Kont_TOitem_b.json")


# ╔═╡ b311137b-3c26-4cd9-94ae-40fa1badd7a9


# ╔═╡ f0278c94-d29a-41ff-9aef-77bcf72f2d62
maxCont = argmax(c -> c[2],factors)[1]

# ╔═╡ aa014b8d-3916-4939-a082-633db51473c3
filter(measuredContainers) do c
	c.container_id === maxCont
end

# ╔═╡ 068d58de-4102-49fb-b290-133e4a85c604
md"## The problem ?"

# ╔═╡ e00d9965-c039-4ae5-8f9c-f72a77c3dde0
md"""
Try to identify the domain. The missing variable im trying to predic and such.

Do a voronoi segemntation! 
"""
