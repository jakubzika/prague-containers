...
# ╔═╡ 82d4b52a-aba2-11ec-0856-83d3d43ba057
begin
...
# ╔═╡ 89f7f89e-3309-4a3a-b833-1e2c01de261f
begin
	include("./src/data-conversion.jl")
	
end

# ╔═╡ afba4963-64a8-4be2-92b9-243242f84c9c
...
# ╔═╡ a4226959-99e0-41a3-9013-5c20c7f9a41d
osmMap = get_map_data("map.osm")

# ╔═╡ 683b343c-489b-446b-9378-dfa28136ea78
pathof(OpenStreetMapX)

# ╔═╡ c56d07c9-3619-47f6-b63e-d2067fa47040
ne(osmMap.g)

# ╔═╡ d9b0d2d1-e86f-46d2-8e1f-005830176501
begin
	flm = pyimport("folium")
...
# ╔═╡ 63de3cad-f225-4206-946b-d0704cc799ae
containers = GeoJSON.parsefile("downloaded-data/containers-geojson/containers.geojson")

# ╔═╡ e79e9ee3-6fde-46f1-894f-b578265e0dad
filteredContainers = filter(features(containers)) do container
	properties(container)["trash_type"]["id"] == 6 && properties(container)["accessibility_id"] === 1
end

# ╔═╡ f1ffc7fe-7499-495b-8eed-cbd3a2547995
begin
	# point1 = (50.072856, 14.436067)
	# point2 = (50.071039, 14.432132)

	point1 = (50.073368, 14.431503)
	point2 = (50.071612, 14.429574)

	mapPoint1 = point_to_nodes(LLA(point1[1],point1[2]), osmMap)
	mapPoint2 = point_to_nodes(LLA(point2[1],point2[2]), osmMap)
end

# ╔═╡ 1e7af333-64ff-476e-8e9e-14c8f01c633f
reverse.(coordinates(geometry.(features(containers))))

# ╔═╡ 697fda43-2f3a-45d5-97ab-d470b8321474


# ╔═╡ f1b57eb5-3555-46de-90c9-ada1a1651014
nodes, distance, speed = shortest_route(osmMap, mapPoint1, mapPoint2)

# ╔═╡ 2bf4a9a5-b269-4efe-a49a-e6b10392c79b
begin
	m = flm.Map()
	
	flm.Marker(point1).add_to(m)

	# flm.Marker(point2).add_to(m)

	for i in features(containers)[1:100]
		p = (geometry(i).coordinates[2],
			geometry(i).coordinates[1])
			
		n = point_to_nodes(LLA(p[1], p[2]), osmMap)
			
		# flm.Marker(p).add_to(m)

		nodes, distance, speed = shortest_route(osmMap, mapPoint1, n)
		locations = [LLA(osmMap.nodes[n], osmMap.bounds) for n in nodes]
...
# ╔═╡ 9291cc6a-62bb-48b6-b2d3-d6fb1a2f7853
# buildings = ArchGDAL.("/Users/jakubzika/School/Bachelor-thesis/Explorations/downloaded-data/filtered-buildings/buildings_without_private_containers_Plast_6.geojson")

# ╔═╡ 1b936305-a93f-4ce7-8454-547e9528909c
buildings = GDF.read("/Users/jakubzika/School/Bachelor-thesis/Explorations/downloaded-data/filtered-buildings/buildings_without_private_containers_Plast_6.geojson")

# ╔═╡ 5984bc7a-de16-4b17-ab3e-a3638edfc7c5
containersPoints = 
	@pipe filteredContainers |>
	geometry.(_) |>
	coordinates.(_) |>
	reverse.(_) |>
	map(x -> SVector(x[1], x[2]), _)

# ╔═╡ 73664d27-b550-4ec1-9bc2-0612f57e52d4
begin
	balltree = BallTree(containersPoints, Haversine(),reorder=true)
end

# ╔═╡ af5cc9fe-32fd-435a-ad81-616e30264260
nearest, distances = knn(balltree,SVector(point2[1],point2[2]),50)

# ╔═╡ 55c6ca1a-3509-4832-97f7-70bac4ccbc61
nearestContainers = filteredContainers[nearest]

# ╔═╡ a750b1b9-ebc2-49d5-b8ff-56143cd0d001
unique([properties(c)["trash_type"]["id"] for c in eachrow(filteredContainers)])

# ╔═╡ 12242e75-371f-47ac-846d-b045278c336b
point = nearestContainers[1] |> geometry |> coordinates |> reverse |> Tuple

# ╔═╡ f2abd5c7-6ede-48b8-86ee-264c3409eece
map(zip(nearest,distances)) do c
	n,d = c
	c
end

# ╔═╡ 50b984b3-fc53-493b-b730-8c1e6ceea6e1
begin
	n = flm.Map()
	50.072548, 14.432085
	n.fit_bounds([(50.07,14.43),(50.074,14.434)])

	flm.Marker(point1, icon=flm.Icon(color="green")).add_to(n)
	
	for container in nearestContainers
		point = container |> geometry |> coordinates |> reverse |> Tuple
		flm.Marker(point, icon=flm.Icon(color="blue"),).add_to(n)

		node = point_to_nodes(LLA(point[1], point[2]), osmMap)
			
		# flm.Marker(p).add_to(m)

		nodes, distance, speed = shortest_route(osmMap, mapPoint1, node)
		locations = [LLA(osmMap.nodes[n], osmMap.bounds) for n in nodes]
...
git-tree-sha1 = "79b3faef10310dfcf3c90f15861d3b2d90cea034"
...
git-tree-sha1 = "69647b9447b5cbc0ab11ca54c8d209b0aeb903fe"
