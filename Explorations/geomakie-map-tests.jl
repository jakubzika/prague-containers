...
# ╔═╡ 9de57d7e-ab73-11ec-08a7-2974fae3c50c
begin
	using Pipe
	using GeoJSON
	using Makie
...
# ╔═╡ 104f6ee5-3b2a-4e21-91e1-5aa678f0cdee
begin
	fontsize_theme = Theme(fontsize = 34)
	set_theme!(fontsize_theme)
end


# ╔═╡ 7785311e-9f77-4d29-90d7-8ed7423265a0
...
# ╔═╡ ed8337da-a466-4315-90b7-6cb88f1cba7b
xdata = let
...
# ╔═╡ cca8a485-7099-4063-8a76-cd10bbb3916a
typeof(geometry(features(data)[1]))

# ╔═╡ 13185d88-ba68-46dd-8364-d46a973688e0
begin
	lDf = DataFrame(geometry=Polygon[],ID=Int64[], count=Int64[])
	# props = properties.(features(data))
	for f in features(data)
		p = properties(f)
		g = geometry(f)
		id = Base.parse(Int, p["ID_2"])
		count = populationCounts[only(findall(==(id), populationCounts.ID)), :].count
		push!(lDf,[g,id, count])
		
	end
end

# ╔═╡ 3627f080-11e8-494a-adaf-4c6922040898
lDf

# ╔═╡ 64cc2ad9-7100-4ccb-88f1-98610eed8507


# ╔═╡ bfbc4f56-f54d-40db-b2a7-43abfd26b07d
len = length(features(data))

# ╔═╡ 53cd9b3d-d1ae-41e0-924a-b1da195907a8
begin
	color = cgrad(:amp, rev=false)
	lons = -180:180
	lats = -90:90
	
	fig = Figure(resolution=(1500,900), font="CMU Serif Roman")
	ax = GeoAxis(fig[1,1],
		# transformation = Proj4.Transformation("EPSG:4326", "+proj=utm +zone=32 +datum=WGS84")
		#dest="+proj=longlat +datum=WGS84 +no_defs"
	)
	
	xlims!(14.22,14.71)
	ylims!(49.9,50.2)
	plot = poly!(ax, data,color=lDf.count, strokecolor = :black,colormap = color, strokewidth=1.0)

	popMin = minimum(lDf.count)
	popMax = maximum(lDf.count)
	...
	colorbar = Colorbar(
		...
		tickformat = "{1:d}",
		ticks=0:10_000:popMax
	)
end;

# ╔═╡ 70c57ed6-c3b6-4c71-af4e-c95f808454a1
fig

# ╔═╡ f50c2bc8-cee0-4809-82b8-ffaa4508ae61
typeof(xdata)

# ╔═╡ daa5806e-b124-4ba4-94d3-d4e277b116fe
a = FeatureCollection()

# ╔═╡ c7a2e445-b8c5-4cbf-b68e-5230aec993ba
append!(features(xdata),features(xdata)[1:2])
