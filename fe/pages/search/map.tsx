...
import { useNearestLocations } from '../../services/apiQueries'
import { useEffect } from 'react'
import NoSSR from 'react-no-ssr'

const NoSSrSearchMap = () => {
  const { data: locations } = useNearestLocations()

  return (
    <ContainersMap
      showFilterWindow={false}
      locations={locations}
      geolocateUserOnLoad={true}
    />
  )
}
    ...
    <div className="mt-2 space-y-4 md:mt-6">
      <StackHeading>
        <div className="flex flex-col md:flex-row">
          ...
          <div>
            <NavLink className="capitalize" href="/search">
              {t('search')}
            </NavLink>
            /
          </div>
